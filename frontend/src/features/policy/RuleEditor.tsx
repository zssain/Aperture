...
export function RuleEditor({ rules, live, errors, onChange }: { rules: PolicyRules; live?: PolicyRules; errors: string[]; onChange: (rules: PolicyRules, field: string) => void }) {
  return <div className="space-y-6 p-5">
    <div className="grid grid-cols-2 gap-4">{NUMERIC_FIELDS.map(([field, label, step]) => {
      const value = rules[field] as number; const changed = live?.[field] !== value; const error = relatedError(field, errors);
